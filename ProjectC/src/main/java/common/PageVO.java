...
public class PageVO {
	...
	int blockPage = 10;			// 블록당 보여질 페이지의 수
	int curPage;				// 현재 페이지
	...
	private String viewType="list";	// 게시판 형태 (기본 : 목록 형태 (list))
	...
	public void setTotalList(int totalList) {
		this.totalList = totalList;   // DB에서 받아온 총 글의 건수 (1536)
		totalPage = totalList / pageList; 	// 총 페이수 = 총 건수 / 페이지당 보여질 글 수
		if ( totalList % pageList > 0 ) ++totalPage; // 총 페이지수 : 154
		totalBlock = totalPage / blockPage;	// 총 블록수 = 총 페이지수 / 블록당 보여질 페이지 수
		if ( totalPage % blockPage > 0 ) ++totalBlock;	// 총 블록 수 : 16
